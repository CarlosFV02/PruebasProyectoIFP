...
const SHIPS = [
  { name: "Portaaviones", size: 5, image: "/static/ships/portaaviones.png" },
  { name: "Acorazado", size: 4, image: "/static/ships/acorazado.png" },
  { name: "Crucero", size: 3, image: "/static/ships/crucero.png" },
  { name: "Submarino", size: 3, image: "/static/ships/submarino.png" },
  { name: "Destructor", size: 2, image: "/static/ships/destructor.png" }
];
...
function renderLayoutState() {
  const app = document.querySelector('.streamlit-like-app');
  if (!app) return;
  app.classList.remove('phase-placement', 'phase-battle', 'phase-finished');
  if (state.phase === 'placement') app.classList.add('phase-placement');
  else if (state.phase === 'battle') app.classList.add('phase-battle');
  else app.classList.add('phase-finished');
}
...
function updateResponsiveBoardSize() {
  ...
  const app = document.querySelector('.streamlit-like-app');
  if (!app) return;
  const appWidth = app.clientWidth || window.innerWidth || 1200;
  let cell = 34;
  let coord = 24;
  if (state.phase === 'placement') {
    if (appWidth < 1300) cell = 30;
    if (appWidth < 1180) cell = 28;
    if (appWidth < 1080) cell = 26;
  } else {
    if (appWidth < 1400) cell = 30;
    if (appWidth < 1250) cell = 28;
    if (appWidth < 1150) cell = 26;
    if (appWidth < 1050) cell = 30;
  }
  root.style.setProperty('--cell-size', `${cell}px`);
  root.style.setProperty('--coord-size', `${coord}px`);
}
...
window.addEventListener("resize", () => { updateResponsiveBoardSize(); renderAll(); });
